...
L.Google = L.Class.extend({
  ...
  options: {
    ...
    maxZoom: 18,
    tileSize: 256,
    dragging: $(document).width() > 480 ? true : false,
    subdomains: 'abc',
    errorTileUrl: '',
    attribution: '',
    opacity: 1,
  ...
  onRemove: function(map) {
    ...
    // Orignal code did this
    // map._container.removeChild(this._container);

    $(`#${this.options.googleMapHolder}`).append($(this._container).detach());
  ...
  _initContainer: function() {
    ...
    if (!this._container) {
      ...
      var leafletGoogleLayer = $(`#${this.options.googleMapHolder} .leaflet-google-layer`);
      if (leafletGoogleLayer.length > 0) {
        this._container = leafletGoogleLayer.detach()[0];
      } else {
  ...
  _initMapObject: function() {
    ...
    var map = window.timbuktu[this.options.googleMapInstanceName];
    // debugger;
    ...
    if (!Ember.isPresent(map)) {
      ...
      window.timbuktu[this.options.googleMapInstanceName] = map;
    }
